...
function MyStoresScreen() {
    ...
    const { myStores, loading: myStoresLoading } = useSelector(state => state.storeMyList)
    const { stocks } = useSelector(state => state.stockList)
    const { success: storeDeleteSuccess, loading: storeDeleteLoading, error } = useSelector(state => state.storeDelete)
...
const closeStockHandler = (index) => {
  ...
  return (
    <>   
        ...
        <Table striped hover responsive className='table-sm my-3' style={{ width:'90%', margin:'auto'}}>
            ...
            <tbody>
                    {myStores.map((store, index) => (
                            ...
                            { button[index] && (
                                    < MyStoreStocks stocks={stocks} store={store} searchHandler={searchHandler} value={value} />
                                )
                            }
                            </>
                     ))}
      ...
      { storeDeleteSuccess && < Notification status='success' message='Store Deleted Successfully!' /> }
      { createStoreSuccess && < Notification status='success' message='Store Created Successfully!'/> }
    </>
  )
}
