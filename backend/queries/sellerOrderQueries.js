...
const getSellerOrder = async (id) => {
  const query = `
    SELECT base_sellerorder.*,
       ...
       COALESCE(json_agg(DISTINCT jsonb_build_object('id', base_instoreorderitem.id, 'name', base_instoreorderitem.name, 'quantity', base_instoreorderitem.quantity, 'price', base_instoreorderitem.price, 'image', base_instoreorderitem.image, 'orderType', base_instoreorderitem."orderType", 'isRetrieved', base_instoreorderitem."isRetrieved", 'retrievedAt', base_instoreorderitem."retrievedAt", 'product', base_instoreorderitem.product_id, 'sellerOrder', base_instoreorderitem."sellerOrder_id", 'store', base_instoreorderitem.store_id, 'details', jsonb_build_object('id', base_product.id, 'brand', base_product.brand, 'name', base_product.name, 'price', base_product.price, 'description', base_product.description, 'image', base_product.image, 'rating', base_product.rating, 'numReviews', base_product."numReviews", 'createdAt', base_product."createdAt", 'category', base_product."category_id", 'seller', base_product.seller_id, 'subcategory', base_product."subcategory_id"), 'store', jsonb_build_object('id', base_store.id, 'owner_name', store_owner.username, 'name', base_store.name, 'address', base_store.address, 'country', base_store.country, 'city', base_store.city, 'latitude', base_store.latitude, 'longitude', base_store.longitude, 'description', base_store.description, 'phone', base_store.phone, 'image', base_store.image, 'owner', base_store.owner_id))::JSONB) FILTER (
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   WHERE base_instoreorderitem.id IS NOT NULL), '[]'::JSON) AS "inStoreOrderItems",
    ...
    (SELECT id,
            username
    FROM auth_user) AS store_owner ON store_owner.id = base_store.owner_id
